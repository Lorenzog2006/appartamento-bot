...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if mode == "subscribe" and token == "juanlespins2026":
            self.send_response(200)
            self.end_headers()
            self.wfile.write(challenge.encode() if challenge else b"ok")
            return

        # Debug: mostra cosa ha ricevuto
        self.send_response(200)
        self.end_headers()
        self.wfile.write(f"path={self.path}|mode={mode}|token={token}|expected=juanlespins2026".encode())
